...
`
	...
	Context("Cluster with minimal settings and orphan iop", func() {
		...
		It("Hook must execute successfully", func() {
			Expect(f).To(ExecuteSuccessfully())
			Expect(string(f.Logger.Contents())).ToNot(HaveLen(0))
			Expect(f.KubernetesGlobalResource("Namespace", "d8-istio").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("Namespace d8-istio deleted"))
			Expect(f.KubernetesResource("IstioOperator", "d8-istio", "v1x16").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("Finalizers from IstioOperator/v1x16 in namespace d8-istio removed"))
			Expect(string(f.Logger.Contents())).To(ContainSubstring("IstioOperator/v1x16 deleted from namespace d8-istio"))
		})
	})
	...
	Context("Cluster with minimal settings and orphan istio resources", func() {
		...
		It("Hook must execute successfully", func() {
			...
			Expect(string(f.Logger.Contents())).To(ContainSubstring("ClusterRole/istio-reader-d8-istio deleted"))
			Expect(f.KubernetesGlobalResource("ClusterRole", "istio-reader-clusterrole-v1x16-d8-istio").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("ClusterRole/istio-reader-clusterrole-v1x16-d8-istio deleted"))
			Expect(f.KubernetesGlobalResource("ClusterRoleBinding", "istio-reader-d8-istio").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("ClusterRoleBinding/istio-reader-d8-istio deleted"))
			Expect(f.KubernetesGlobalResource("ClusterRoleBinding", "istio-reader-clusterrole-v1x16-d8-istio").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("ClusterRoleBinding/istio-reader-clusterrole-v1x16-d8-istio deleted"))
			Expect(f.KubernetesGlobalResource("MutatingWebhookConfiguration", "istio-sidecar-injector-v1x16-d8-istio").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("MutatingWebhookConfiguration/istio-sidecar-injector-v1x16-d8-istio deleted"))
			Expect(f.KubernetesGlobalResource("ValidatingWebhookConfiguration", "istio-validator-v1x16-d8-istio").Exists()).To(BeFalse())
			Expect(string(f.Logger.Contents())).To(ContainSubstring("ValidatingWebhookConfiguration/istio-validator-v1x16-d8-istio deleted"))
		})
	})
