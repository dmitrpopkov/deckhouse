...
`
	...
	Context("Two proper pairs of NG+IC and a NG with wrong ref kind", func() {
		...
		It("Proper NGs must be stored to nodeManager.internal.nodeGroups, hook must warn user about improper NG", func() {
			...
			Expect(f.Logger).Should(gbytes.Say("Wrong classReference: Kind ImproperInstanceClass is not allowed, the only allowed kind is D8TestInstanceClass."))
...
`))
		...
		It("Proper NGs must be stored to nodeManager.internal.nodeGroups, old improper NG data must be saved, hook must warn user about improper NG", func() {
			...
			Expect(f.Logger).Should(gbytes.Say("Wrong classReference: Kind ImproperInstanceClass is not allowed, the only allowed kind is D8TestInstanceClass. Earlier stored version of NG is in use now!"))
	...
	Context("Two proper pairs of NG+IC and a NG with wrong ref name", func() {
		...
		It("Proper NGs must be stored to nodeManager.internal.nodeGroups, hook must warn user about improper NG", func() {
			...
			Expect(f.Logger).Should(gbytes.Say(`Wrong classReference: There is no valid instance class improper of type D8TestInstanceClass.`))
	...
	Context("Two proper pairs of NG+IC and a NG with wrong zones", func() {
		...
		It("Proper NGs must be stored to nodeManager.internal.nodeGroups, hook must warn user about improper NG", func() {
			...
			Expect(f.Logger).Should(gbytes.Say(`unknown cloudInstances\.zones: \[xxx\]`))
...
`))
		...
		It("Proper NGs must be stored to nodeManager.internal.nodeGroups, old improper NG data must be saved, hook must warn user about improper NG", func() {
			...
			Expect(f.Logger).Should(gbytes.Say(`Wrong classReference: There is no valid instance class improper of type D8TestInstanceClass. Earlier stored version of NG is in use now!`))
...
`))
		...
		It("NodeGroup values must be valid", func() {
			...
			Expect(string(f.Logger.Contents())).To(ContainSubstring("Calculate capacity failed for: D8TestInstanceClass"))
		})
	})
