...
`))
		...
		It("Hook must execute successfully with proper warnings", func() {
			...
			Expect(string(f.Logger.Contents())).To(Not(ContainSubstring("local-federation")))

			Expect(string(f.Logger.Contents())).To(ContainSubstring("cannot fetch public metadata endpoint https://public-internal-error/metadata/public/public.json for IstioFederation public-internal-error (HTTP Code 500)"))
			Expect(string(f.Logger.Contents())).To(ContainSubstring("cannot unmarshal public metadata endpoint https://public-bad-json/metadata/public/public.json for IstioFederation public-bad-json, error: unexpected end of JSON input"))
			Expect(string(f.Logger.Contents())).To(ContainSubstring("bad public metadata format in endpoint https://public-wrong-format/metadata/public/public.json for IstioFederation public-wrong-format"))
			Expect(string(f.Logger.Contents())).To(ContainSubstring("cannot fetch private metadata endpoint https://private-internal-error/metadata/private/federation.json for IstioFederation private-internal-error (HTTP Code 500)"))
			Expect(string(f.Logger.Contents())).To(ContainSubstring("cannot unmarshal private metadata endpoint https://private-bad-json/metadata/private/federation.json for IstioFederation private-bad-json, error: unexpected end of JSON input"))
			Expect(string(f.Logger.Contents())).To(ContainSubstring("bad private metadata format in endpoint https://private-wrong-format/metadata/private/federation.json for IstioFederation private-wrong-format"))
