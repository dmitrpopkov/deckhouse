...
Эта конфигурация позволит так же запускать некоторую полезную нагрузку, при этом после установки Deckhouse будет занимать примерно:
- ~25% CPU, ~9 ГБ RAM, ~20 ГБ дискового пространства

В такой конфигурации при нагрузке в 2500rps на условное веб-приложение (статическая страница nginx) из 30 подов и входящем трафике в 24 Мбит/с:
